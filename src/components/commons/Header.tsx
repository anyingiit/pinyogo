...
import style from "../../css/commons/header.module.scss"
...
export default class Header extends React.Component<Props, any> {
  ...
  render() {
    return (
      <div className={[style.header, 'w'].join(' ')}>
        {/* logo */}
        <div className={style.ComponentLogo} style={this.props.showLogo ? {display: 'block'} : {display: 'none'}}>
          <Logo smallPic={this.props.logoSmailPic} smallTitle={this.props.logoSmailTitle}/>
        </div>
        {/* search */}
        <div className={style.search} style={this.props.showSearch ? {display: 'block'} : {display: 'none'}}>
          <input type="text" placeholder="请搜索"/>
          <button>搜索</button>
        </div>
        {/* hot-words */}
        <div className={style.hotWords} style={this.props.showHotWords ? {display: 'block'} : {display: 'none'}}>
